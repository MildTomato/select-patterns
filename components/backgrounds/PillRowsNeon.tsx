...
interface Row { y:number;rowIdx:number;slots:Slot[];spans:Span[];segs:Seg[] }
...
// Blocks variation driven by the shared palette + animation controls.
const N_COLORS = 4
const QL = 32
...
export default function PillRowsNeon() {
  ...
  useEffect(()=>{
    ...
    const resize = ()=>{
      ...
      for(let r=0;r<nRows;r++){
        ...
        rows.push({y:r*ROW_SPACING+ROW_SPACING/2,rowIdx:r,slots,spans,segs:[]})
      }
      rowsR.current=rows
      blobsR.current=[
    ...
    const frame=(now:number)=>{
      ...
      for(const row of rowsR.current){
        ...
        // Slide mode sways harder and faster — the sliding IS the show there
        ...
        if(mode==="slide"){
          // Segment model: find the row's active runs, tween the run
          // boundaries so lit stretches glide along the row, and step the
          // pills only while a run is being created or destroyed
          const slots=row.slots
          const targets:{L:number;R:number}[]=[]
          let start=-1
          for(let i=0;i<=slots.length;i++){
            const cx=i<slots.length ? slots[i].x+(UNIT-GAP)/2 : 0
            const on = i<slots.length && influenceAt(cx+drift,cy,cx) > 0.30+rowBias
            if(on){ if(start<0) start=i }
            else if(start>=0){
              targets.push({L:slots[start].x, R:slots[i-1].x+UNIT-GAP})
              start=-1
            }
          }
          // Match targets to live segments by center distance
          ...
          for(const sg of row.segs){
            ...
            // Lag behind the moving target — the visible glide
