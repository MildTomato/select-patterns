...
export default function TextArtGeo() {
  ...
  const [sizeMul, setSizeMul] = usePersisted("geo-size",1)
  const [speedMul, setSpeedMul] = usePersisted("geo-speed",1)
  const [spinMul, setSpinMul] = usePersisted("geo-spin",1)
  const [pulseMul, setPulseMul] = usePersisted("geo-pulse",1)
  ...
  const [showText, setShowText] = usePersisted("geo-show-text",true)
  const [textSize, setTextSize] = usePersisted("geo-text-size",1)
  const [rerender, setRerender] = usePersisted("geo-rerender",false)
  const [rerenderSecs, setRerenderSecs] = usePersisted("geo-rerender-secs",3)
