...
// Film-grain gradient fields: smooth blob fields rendered with per-pixel
// stochastic dithering so color transitions become sandy speckle, not blends.
// Two ramps share black and cream endpoints; a second field mixes them in.
const COOL: [number,number,number][] = [
  [10,10,12],    // black
  [42,36,112],   // deep indigo
  [87,71,224],   // indigo
  [160,107,228], // violet
  [240,154,218], // pink
  [242,239,191], // cream
...
export default function DitherGrain() {
  ...
  useEffect(()=>{
    ...
    const resize = ()=>{
      ...
      blobsG=[
        {x:W*.60,y:H*.85,vx:.16,vy:-.12,angle:0.9, angleSpeed: .002,radiusX:W*.55,radiusY:H*.45},
        {x:W*.95,y:H*.65,vx:-.20,vy:.14,angle:2.3,angleSpeed:-.003,radiusX:W*.50,radiusY:H*.50},
      ]
    }
    ...
    const frame=(now:number)=>{
      ...
      for(let j=0;j<fh;j++){
        ...
        for(let i=0;i<fw;i++){
          const cx=i*CS
          // Compress + gamma so cream stays a rare peak, indigo/violet dominate
      ...
      // Grain pass — stochastic dither between adjacent ramp stops per pixel
      const d=img.data
      const top=COOL.length-1
      let p=0
      for(let y=0;y<lowH;y++){
        ...
        for(let x=0;x<lowW;x++){
          ...
          // Sharpen the ramp mix so green reads as a region with grainy edges
          ...
          const idx=i+(Math.random()<t-i?1:0)
          const c=(Math.random()<g?GREENS:COOL)[idx]
