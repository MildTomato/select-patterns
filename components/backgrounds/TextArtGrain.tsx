...
// Text-art shapes over film grain: the same field rendered three ways —
// a per-pixel stochastic grain underlay, the fine glyph-density grid, and
// large drifting character-shapes on top.
const N_COLORS = 4
const QL = 32
const UNIT = 12
...
// Macro layer: coarse grid of large repeated characters
const MUNIT = 22
const MROW = 28
const MFONT = "bold 20px monospace"
...
export default function TextArtGrain() {
  ...
  useEffect(()=>{
    ...
    const mcache = new Map<string,HTMLCanvasElement>()
    const mglyph = (ch:string, fill:string)=>{
      const key=ch+"|"+fill
      let c=mcache.get(key)
      if(!c){
        const dpr=window.devicePixelRatio||1
        c=document.createElement("canvas")
        c.width=MUNIT*dpr; c.height=MROW*dpr
        const g=c.getContext("2d")!
        g.scale(dpr,dpr)
        g.font=MFONT
        g.textAlign="center"; g.textBaseline="middle"
        g.fillStyle=fill
        g.fillText(ch,MUNIT/2,MROW/2)
        mcache.set(key,c)
      }
      return c
    }
    ...
    const resize = ()=>{
      ...
      // Big interweaving character-shapes — they wrap, so they keep
      // drifting in and out of frame
      macro=[
        {ch:"[",x:W*.25,y:H*.30,vx:.34,vy:.10,angle:.3, va:.0009, rx:W*.26,ry:H*.32,color:3},
        {ch:"{",x:W*.72,y:H*.62,vx:-.26,vy:.16,angle:1.1,va:-.0007,rx:W*.21,ry:H*.36,color:0},
        {ch:"|",x:W*.50,y:H*.15,vx:.18,vy:-.22,angle:2.0,va:.0008, rx:W*.30,ry:H*.22,color:2},
        {ch:"=",x:W*.85,y:H*.82,vx:-.38,vy:-.09,angle:.8, va:-.0010,rx:W*.23,ry:H*.27,color:1},
      ]
    }
    ...
    const frame=(now:number)=>{
      ...
      // Macro layer: each big shape stamps its character over the field;
      // where shapes overlap, the strongest wins — interweaving forms
      const MCOLS=Math.ceil(W/MUNIT), MROWS2=Math.ceil(H/MROW)
      for(let r=0;r<=MROWS2;r++){
        const my2=r*MROW+MROW/2
        for(let c=0;c<=MCOLS;c++){
          const mx2=c*MUNIT+MUNIT/2
          let best=-1, bi=0
          for(let k=0;k<macro.length;k++){
            const m=macro[k]
            const dx=mx2-m.x,dy=my2-m.y
            const cos=Math.cos(m.angle),sin=Math.sin(m.angle)
            const lx=dx*cos+dy*sin,ly=-dx*sin+dy*cos
            const d=1-Math.sqrt((lx/m.rx)**2+(ly/m.ry)**2)
            if(d>best){ best=d; bi=k }
          }
          if(best>0.04){
            const m=macro[bi]
            ctx.globalAlpha=Math.min(1,best*7)
            ctx.drawImage(mglyph(m.ch,fills[m.color%nf][QL]),c*MUNIT,r*MROW,MUNIT,MROW)
            ctx.globalAlpha=1
          }
        }
      }
