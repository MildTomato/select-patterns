...
// 8x8 Bayer matrix for much finer dithering pattern
const BAYER8 = [
...
export default function DitherHeavy() {
  ...
  useEffect(()=>{
    ...
    const STEP = 22 // smaller cells for denser Bayer pattern
    ...
    const frame=(now:number)=>{
      ...
      // Mid-gray background so both dark and light cells contrast
      ...
      for(let row=0;row<=ROWS;row++){
        for(let col=0;col<=COLS;col++){
          ...
          // 8x8 Bayer — much finer dither texture
          const bayer = BAYER8[row%8][col%8] / 64
          const inside = inf > 0.30 + (bayer - 0.5) * 0.55 // wider spread = more dithering
